...
func (dk *BuiltinKnowledge) loadSequential(
	...
	for i, src := range sources {
		...
		for j, doc := range docs {
			...
			// Respect progressStepSize for reporting frequency.
...
func (dk *BuiltinKnowledge) loadConcurrent(
	...
	for i, src := range sources {
		...
		err := srcPool.Submit(func() {
			...
			log.InfofContext(ctx, "Loading source %d/%d: %s (type: %s)",
				sourceIndex, sourceTotal, sourceName, sourceType)
			...
			log.InfofContext(ctx, "Fetched %d document(s) from source %s", len(docs), sourceName)
			if err := dk.processDocuments(ctx, docs, config, aggr, docPool, source, sourceIndex, sourceTotal); err != nil {
